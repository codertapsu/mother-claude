...
.DESCRIPTION
  Windows counterpart of scripts/release/release-upload.sh. Pairs with a local
  build so cutting a release doesn't burn (10x-billed) macOS Actions minutes:
    npm ci
    npm run tauri:build
    pwsh scripts/release/release-upload.ps1 -Upload `
...
$Repo = if ($env:GH_REPO)     { $env:GH_REPO }     else { 'codertapsu/mother-claude' }
...
function Ensure-Release {
  ...
  if ($r) { return $r.id }
  $body = @{ tag_name = $Tag; name = "Mother Claude $Tag"; draft = $true; prerelease = $false } | ConvertTo-Json
...
function Upload-Asset($rid, $file) {
  # Normalize spaces to '.' (matching tauri-action/GitHub asset naming) so the
  # query is a valid URL, re-uploads replace the same asset, and macOS/Windows
  # asset names are consistent.
  $name = (Split-Path $file -Leaf) -replace ' ', '.'
  ...
  $url = "https://uploads.github.com/repos/$Repo/releases/$rid/assets?name=$enc"
